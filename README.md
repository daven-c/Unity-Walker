...
- **Headless parallel training** — build the project to a standalone player (File → Build) and run `mlagents-learn` against it with `--no-graphics --num-envs=N`. This is the main remaining throughput win, because the bottleneck is **single-threaded**: Unity's main thread runs the entire `FixedUpdate` loop (physics → decisions → rewards), so on a 16-thread machine a fully saturated main thread reads as only ~20–30% aggregate CPU while a dozen threads idle. Raising `time_scale` or adding platforms to the scene both pile onto that same thread and hit the same ceiling — past it, the Editor just lags while simulated throughput plateaus. `--num-envs` is different: each environment is a separate OS process with its own main thread, so it actually spreads across cores. On an 8-core machine, budget ~1 core for the Python trainer and one for the OS, then try `--num-envs=4` and scale from there while watching per-core utilization rather than the aggregate number.
